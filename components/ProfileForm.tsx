...
export default function ProfileForm({
  ...
  return (
    <>
      <div className="glass rounded-2xl p-8 max-w-2xl mx-auto w-full">
        <div className="text-center mb-8">
          <div className="w-20 h-20 mx-auto rounded-2xl gradient-accent flex items-center justify-center shadow-lg mb-4 animate-float">
            <span className="text-black font-black text-2xl">{nama.charAt(0).toUpperCase()}</span>
          </div>
          <h2 className="text-xl font-bold text-[var(--text-primary)]">{nama}</h2>
          <p className="text-sm text-[var(--text-secondary)] mt-1">{email || "Belum diatur"}</p>
        </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center px-4 animate-fadeIn" style={{ background: "var(--modal-overlay)", backdropFilter: "blur(8px)" }}>
          <div className="glass rounded-2xl p-7 max-w-sm w-full animate-scaleIn border border-[var(--border)]">
            ...
            <p className="text-sm text-[var(--text-secondary)] mb-5">Data profil Anda akan diperbarui.</p>
